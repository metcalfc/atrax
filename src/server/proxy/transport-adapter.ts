...
export class TransportAdapter implements Transport {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Failed to start transport for client ${this.clientId} (session ${this.sessionId}):`,
        error
      );
      throw error;
    }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        `Error stopping transport for client ${this.clientId} (session ${this.sessionId}):`,
        error
      );
      throw error;
    }
  ...
  async send(message: JSONRPCMessage): Promise<void> {
    if (!this.running) {
      logger.error(
        `Cannot send message for client ${this.clientId} (session ${this.sessionId}): Transport not running`
      );
    ...
    try {
      ...
      logger.error(
        `Error sending message to client ${this.clientId} (session ${this.sessionId}):`,
        error
      );
      throw error;
    }
  ...
  on(type: string, handler: (...args: any[]) => void): void {
    logger.debug(
      `Registering ${type} handler for client ${this.clientId} (session ${this.sessionId})`
    );
  ...
  removeListener(type: string, handler: (...args: any[]) => void): void {
    logger.debug(
      `Removing ${type} handler for client ${this.clientId} (session ${this.sessionId})`
    );
  ...
  async handlePostMessage(req: Request, res: Response): Promise<void> {
    if (!this.running) {
      logger.error(
        `Cannot handle message for client ${this.clientId} (session ${this.sessionId}): Transport not running`
      );
      throw new Error('Transport not running');
    }
    ...
    try {
      ...
      logger.error(
        `Error handling POST message for client ${this.clientId} (session ${this.sessionId}):`,
        error
      );
      throw error;
    }

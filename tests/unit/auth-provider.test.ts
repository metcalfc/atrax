...
describe('Auth Provider', () => {
  ...
  describe('createAuthProvider', () => {
    ...
    it('should throw error for token auth without token', () => {
      expect(() => createAuthProvider('token')).toThrow('Token is required for token authentication');
    });
    ...
    it('should register user when creating basic auth with credentials', () => {
      ...
      return provider.authenticate({ userId: 'test', password: 'pass' })
        .then(result => {
          expect(result.success).toBe(true);
    ...
    it('should throw for unsupported auth types', () => {
      expect(() => createAuthProvider('invalid')).toThrow('Unsupported authentication type: invalid');
    });
  });
});
...
/**
 * Test script for the Memory Server using the MCP SDK directly
 * This approach uses the proper SDK mechanisms for client-server communication
 */
...
const SERVER_PATH = path.join(
  ...
  '../dist/src/server/examples/memory-server.js'
);

// Path to the memory file
const MEMORY_FILE_PATH = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  '../dist/src/server/examples/memory.json'
);
...
async function runTests() {
  console.log(`${colors.blue}===== Testing Memory Server =====\n${colors.reset}`);
